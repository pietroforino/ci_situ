...
SECONDI_PER_GIORNO = 0.5  # Portato a 0.5s per poter leggere i log chiaramente
...
def carica_e_prepara_dati():
    ...
    print(f"[MASTER] Colonne lette: {list(df.columns)}")
...
def main():
    ...
    print(f"[MASTER] Inoltro OSC su porta {OSC_PORT} verso {NODI_IP}...\n")
    ...
    try:
        while True:
            ...
            stato_str = f"INCENDIO (FRP/Gravità: {gravita_corrente:.1f})" if fuoco_attivo else "SILENZIO"
            print(
                f"[MASTER TX] {data_corrente.strftime('%Y-%m-%d')} | "
                f"Fuoco: {fuoco_attivo} | Stato: {stato_str:30s} | GravNorm: {gravita_norm:.2f}"
            )
